export default function ResultLabel({ results }) {
  ...
  if (results[0]["class"] && results[0]["class"].startsWith("错误")) {
    return <div className="result-empty" style={{ color: "var(--terracotta)" }}>{results[0]["class"]}</div>
  }
      ...
      {results.map((r, i) => (
        <div key={i} className="result-item">
          ...
          <div className="bar-wrap">
            <div className="bar-fill" style={{ width: `${maxConf > 0 ? (r.confidence / maxConf) * 100 : 0}%` }} />
          </div>
